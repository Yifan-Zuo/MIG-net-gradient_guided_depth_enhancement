...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        #sess.run(init_op)
        saver_full.restore(sess, "/media/yifan/Data/trained_models/inten_grad_guided_depup/noisy/8x/full_models2/8x_ny_igg_model.ckpt-56")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/yifan/Data/trained_models/inten_grad_guided_depup/noisy/8x/full_models3/8x_ny_igg_model.ckpt",global_step=model_ind)
