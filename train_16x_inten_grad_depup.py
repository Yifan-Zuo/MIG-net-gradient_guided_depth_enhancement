...
#HR_gen_depgrad=tf.norm(tf.image.sobel_edges(HR_gen_dep),2,4)

#define loss for gen
loss=tf.reduce_mean(tf.squared_difference(HR_gen_dep,HR_depth_batch_input))
#loss=tf.reduce_mean(tf.abs(HR_gen_dep-HR_depth_batch_input))+0.5*tf.reduce_mean(tf.abs(HR_grad_dep-HR_depgrad_batch_input))+0.5*tf.reduce_mean(tf.abs(HR_gen_depgrad-HR_depgrad_batch_input))
#loss=tf.reduce_mean(tf.abs(HR_gen_dep-HR_depth_batch_input))+tf.reduce_mean(tf.abs(HR_grad_dep-HR_depgrad_batch_input))
...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        #sess.run(init_op)
        saver_full.restore(sess, "/media/yifan/Data/trained_models/inten_grad_guided_depup/noisy/16x/second_train/full_models2/16x_ny_igg_model.ckpt-4")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/yifan/Data/trained_models/inten_grad_guided_depup/noisy/16x/second_train/full_models3/16x_ny_igg_model.ckpt",global_step=model_ind)
